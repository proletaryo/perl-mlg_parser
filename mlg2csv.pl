...
while(<>) {
  ...
  # reached the end, capture data to hash then reset values
    ...
    if (not defined($SystemTraceNumber) or not defined($PrimaryAccountNumber) or not defined($RetrievalReferenceNumber)) {
      next;
    }
    # key to use: SystemTraceNumber+PrimaryAccountNumber+RetrievalReferenceNumber
    my $UniqueKey = $SystemTraceNumber . $PrimaryAccountNumber . $RetrievalReferenceNumber;
    ...
    if (defined($SystemTraceNumber)) {
      $href_LookupData->{$UniqueKey}->{'SystemTraceNumber'} = $SystemTraceNumber;
    }
    if (defined($RetrievalReferenceNumber)) {
      $href_LookupData->{$UniqueKey}->{'RetrievalReferenceNumber'} = $RetrievalReferenceNumber;
    }
    if (defined($TimeLeg1)) {
